            ...
            <div key={group.key}>
              ...
              <ul className="space-y-2">
                {group.items.map((booking) => {
                  const startsAt = DateTime.fromISO(booking.startAt, { zone: 'utc' }).setZone(
                    timezone,
                  );
                  const endsAt = DateTime.fromISO(booking.endAt, { zone: 'utc' }).setZone(
                    timezone,
                  );
                  const isOngoing =
                    startsAt.isValid && endsAt.isValid && startsAt <= now && now < endsAt;
                  ...
                  return (
                    ...
                    <button
                      ...
                      className={`w-full rounded-lg border px-3 py-2 text-left transition ${
                        isOngoing
                          ? 'border-emerald-300 bg-emerald-50 ring-1 ring-emerald-200 hover:bg-emerald-100'
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0 flex-1">
                          ...
                          <p
                            className={`mt-0.5 truncate text-[10px] font-medium uppercase tracking-wide ${
                              isOngoing ? 'text-emerald-700' : 'text-slate-500'
                            }`}
                          >
                        ...
                        {isOngoing ? (
                          <span className="shrink-0 rounded-full bg-emerald-600 px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wide text-white">
                            Live
                          </span>
                        ) : null}
                      </div>
